...
const inactiveBox = 'bg-[#F2F5F8] text-gray-500 hover:shadow-md';
  ...
  return (
    <section className="font-montserrat bg-[#F8FCFF] py-20">
      <div className="mx-auto max-w-5xl px-6 text-center">
        ...
        <div className="mt-14 flex flex-wrap justify-center gap-6">
          {Object.entries(FAQ_DATA).map(([key, { label, icon: Icon }]) => (
            <button
              ...
              className={`flex flex-col items-center w-28 sm:w-32 rounded-2xl p-6
                          transition-all duration-300
                          ${activeCat === key ? activeBox : inactiveBox}`}
            >
              <Icon size={32} />
              <span className="mt-4 text-xs sm:text-sm font-medium leading-tight">
                {label}
              </span>
            </button>
          ))}
        ...
        <div
          key={fadeKey} 
          className="mt-14 bg-white rounded-2xl shadow-xl max-w-3xl mx-auto
                     animate-fade-in"
        >
          {qa.map(([q, a], idx) => {
