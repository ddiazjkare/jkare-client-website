...
export const createShipment = async (shipmentData) => {
  try {
    ...
    const response = await shippoClient.post("/shipments/", {
      ...shipmentData,
      // extra: {
      //   ...(shipmentData.extra || {}), 
...
export const getShipment = async id =>  {
  try{
    const response = await shippoClient.get(`/shipments/${id}`);
    return response.data;
  }catch(error){
    throw error;
  }
...
export const trackShipment = async (carrier, trackingNumber) => {
  try {
    ...
    const tr_num = shippo_key.startsWith("shippo_test")
      ? "shippo_transit"
      : trackingNumber;
    const response = await shippoClient.get(`/tracks/${carrier}/${tr_num}/`);
    return response.data;
  } catch (error) {
